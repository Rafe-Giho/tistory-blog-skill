/**
 * 티스토리 스킨 치환자 카탈로그 — TypeScript catalog for Tistory skin substitutions.
 *
 * 두 용도:
 *  1. `tistory://substitutions` catalog resource 의 source.
 *  2. `skin_validate` 도구가 코드 안 치환자/블록을 대조하는 reference.
 *
 * raw HTML 재파싱 보강 (Odyssey 실측 65개 등) 은 후속 작업 — future raw HTML parsing improvements.
 * 이미지 치환자 `[##_Image|kage@{KEY}|CDM|1.3|{JSON}_##]` 는 the Tistory image substitution format 에서 가져옴.
 */
...
const values: ValueSubstitution[] = [
  ...
  // Post — 이전/다음 글 링크
  ...[
    { token: "[##_article_prev_link_##]", parents: ["<s_article_prev>"], returns: "이전 글 URL" },
    { token: "[##_article_prev_title_##]", parents: ["<s_article_prev>"], returns: "이전 글 제목" },
    { token: "[##_article_next_link_##]", parents: ["<s_article_next>"], returns: "다음 글 URL" },
    { token: "[##_article_next_title_##]", parents: ["<s_article_next>"], returns: "다음 글 제목" },
  ...
  // Sidebar — 사이드바 요소 위젯
  {
    token: "[##_sidebar_element_##]",
    scope: ["sidebar"],
    parents: ["<s_sidebar_element>"],
    returns: "현재 사이드바 요소 위젯 본문",
  },

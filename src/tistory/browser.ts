/**
 * Playwright session manager — interactive Tistory login helper.
 *
 * 카카오 OAuth + 2FA 푸시는 헤들리스로 못 뚫음 (Tistory login flow) → 헤디드 한 번.
 * 그 외 모든 도구는 여기서 캡처한 cookie 로 `src/tistory/api.ts` 의 fetch 만 사용.
 * 다른 모듈에서 절대 Playwright 다시 띄우지 말 것 (avoid duplicate browser sessions).
 *
 * storageState 보관:
 *   - keytar 가 연결하는 OS 자격증명 저장소에 JSON 그대로 암호화 저장
 *   - macOS Keychain, Windows Credential Manager, Linux Secret Service 계열 지원
 *   - service = "tistory-blog", account = blog host (예: "example.tistory.com")
...
const CREDENTIAL_SERVICE = "tistory-blog";
...
export interface LoginOptions {
   ...
   * 예: `"example.tistory.com"` 또는 `"https://example.tistory.com"`.
   */
  blogUrl: string;
  /**
...
export async function loginInteractive(opts: LoginOptions): Promise<LoginResult> {
  ...
  try {
    ...
    const blogId = await page
      .evaluate(() => {
        // @ts-expect-error - window.Config 는 admin 페이지가 inline 으로 박는다 (Tistory admin API notes)
...
/**
 * OS credential store 에 저장된 세션 삭제. blogUrl 미지정 시 `default` 슬롯만.

 ...
 * 티스토리 관리자 API 관찰 결과를 타입과 helper 함수로 정리한 모듈.
 *
 * 모든 호출은 Playwright가 한 번 발급한 cookie만 의존.
...
export interface TistoryContext {
  /** 블로그 호스트. 예: `"example.tistory.com"` (프로토콜 없이) */
  host: string;
...
// ─────────────────────────────────────────────────────────────────────────────
// visibility enum 이중성 — Tistory admin API notes
// ─────────────────────────────────────────────────────────────────────────────
...
// ─────────────────────────────────────────────────────────────────────────────
// 메타 1: window.Config 추출 — Tistory admin API notes
// ─────────────────────────────────────────────────────────────────────────────
 ...
 *
 * 실측 (2026-05, example.tistory.com): top-level 에 `blogId`/`user` 는 더 이상 박히지 않음.
 ...
 * `/manage/category` 가 가장 가벼우면서 안정적 (Tistory admin API notes).
 *
...
// Tistory admin API notes §5
// ─────────────────────────────────────────────────────────────────────────────

/** `/manage/posts.json` 의 item 한 건. Tistory admin API notes */
...
export async function listPosts(
  ...
  // Current Tistory admin rejects `visibility=all` (and PRIVATE/private) with 400.
  // Omitting the parameter returns the all-posts view, including private posts.
...
/**
 * POST/PUT 공통 body. `visibility` 는 정수 (Tistory admin API notes§4.3).
 *
...
/**
 * `POST /manage/post.json` — 신규 발행. Tistory admin API notes
 *
...
/**
 * `PUT /manage/post/{id}.json` — 수정. Tistory admin API notes
 *
...
/** `DELETE /manage/post/{id}.json`. Tistory admin API notes */
...
/** 이미지 업로드 응답 — Tistory admin API notes */
...
/** 본문 삽입용 이미지 정렬/크기 메타. Tistory admin API notes */
...
/**
 * 업로드 응답을 본문에 박을 영구 치환자로 변환. Tistory admin API notes
 *
...
/** `GET /manage/design/skin/html.json` 응답. Tistory admin API notes */
 ...
 *   `isPreview: false` 는 즉시 라이브 발효 (Tistory admin API notes).
 *
...
/** `GET /manage/design/skin/current.json` 응답. Tistory admin API notes */
 ...
 * 부분 패치 흉내내려면 `getSkinCurrent` 로 받아서 머지 후 전체 박아라 (Tistory admin API notes).
 */
...
/** preview 페이지 enum. Tistory admin API notes */
...
/**
 * `POST /preview/skin/{page}` — 서버 렌더 풀 HTML. Tistory admin API notes§6.4.
 *
...
/** `GET /manage/category.json` 응답. Tistory admin API notes */
...
/** `PUT /manage/category.json` 응답. Tistory admin API notes (GET 과 키 이름 다름 — `categoryTree`). */
...
/**
 * `PUT /manage/category.json` body — 3-array diff. Tistory admin API notes
 *
 * **함정:**
...
/** PUT `append[]` 각 객체. Tistory admin API notes */
...
/** PUT `update[]` 각 객체. Tistory admin API notes */
...
/**
 * `PUT /manage/category.json` — 카테고리 batch CRUD. Tistory admin API notes
 *
